...
const WalletCreateModel = require('./model.js');
...
module.exports = class WalletCreateViewModel extends ViewModel {
    ...
    onWizardToPassword(e) {
        ...
        let cpw = $('input[name="text-password-confirm"]');
        if (pw.val() != cpw.val()) {
            alert('The password was not confirmed properly.');
            return;
        }
        //update model
        this.model.password = pw.val();
        this.model.keyFilePath = $('.button-key-file').data('file-name') || null;
        //move to next step
        self.wizardStep('wizard-step-password');
    }
    ...
    onWizardToRecovery(e) {
        ...
        //move to next step
        self.wizardStep('wizard-step-recovery');
    }
    ...
    onWizardToAccounts(e) {
        ...
        let recoveryEnabled = ($('.wizard .wizard-step-recovery input[type="checkbox"]').is(':checked'));
        //move to next step
        self.wizardStep('wizard-step-accounts');
    }

    wizardStep(nextStepClassName) {
        $(`.wizard .wizard-step:not(.${nextStepClassName})`).fadeOut(function () {
            $(`.wizard .wizard-step:not(.${nextStepClassName})`).hide();
            $(`.wizard .${nextStepClassName}`).fadeIn();
        });
    }
